...
export default class Pagination {
  ...
  #createModeHandler(root, mode, datasetHelper, callback) {
    switch(mode) {
      case 'button':
        return new ButtonMode(root, callback, datasetHelper)
      case 'more':
        return new MoreMode(root, callback, datasetHelper)
    }
  }
}
...
class MoreMode {
  ...
  constructor(root, callback, datasetHelper) {
...
class ButtonMode {
  ...
  setPage(pageProp) {
    const size = pageProp.size
    const currentPage = parseInt(pageProp.number) || 0
    const totalPage = pageProp.totalPages - 1 || 0
    const startPage = Math.max(0, currentPage - 2)
    const endPage = Math.min(totalPage, startPage + 4)
    
    this.reset()
    this.#pageStat = { size, currentPage, totalPage }
